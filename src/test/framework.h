...
static void assert_eq(int line, u32 actual, u32 expected) {
    g_tests_run++;
    if (actual != expected) {
        g_tests_failed++;
...
static void assert_true(int line, int condition) {
    g_tests_run++;
    if (!condition) {
        g_tests_failed++;
...
static void assert_false(int line, int condition) {
    g_tests_run++;
    if (condition) {
        g_tests_failed++;
...
static void assert_str_eq(int line, const char *actual, const char *expected) {
    g_tests_run++;
...
static void run_tests(const char *group_name, test_fn tests[]) {
    printf(COLOR_YELLOW "=== %s ===" COLOR_RESET "\n", group_name);
