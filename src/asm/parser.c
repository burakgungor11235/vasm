 ...
 */
static u32 add_literal_pool_entry(program_state_t *prog, u32 value) {
  ...
  if (prog->literal_pool_count < MAX_LITERAL_POOL_ENTRIES) {
    ...
    prog->literal_pool[prog->literal_pool_count].offset = 0;  /* Will be set by emit_literal_pool */
 ...
 */
static void emit_literal_pool(program_state_t *prog) {
  ...
  for (int i = 0; i < prog->literal_pool_count; i++) {
    ...
    fprintf(stderr, "DEBUG:   pool[%d]: value=0x%08X, offset=%u\n",
            i, prog->literal_pool[i].value, prog->literal_pool[i].offset);
  }
  ...
  for (int j = 0; j < prog->literal_pool_ref_count; j++) {
    ...
    if (pool_idx >= 0 && pool_idx < prog->literal_pool_count) {
      ...
      fprintf(stderr, "DEBUG:   old_instr=0x%08X (opcode=0x%02X, cond=0x%X, rn=0x%X, rd=0x%X, off=0x%03X)\n",
              old_instr, (old_instr >> 24) & 0xFF, (old_instr >> 20) & 0xF,
              (old_instr >> 16) & 0xF, (old_instr >> 12) & 0xF, old_instr & 0xFFF);
      fprintf(stderr, "DEBUG:   new_instr=0x%08X (opcode=0x%02X, cond=0x%X, rn=0x%X, rd=0x%X, off=0x%03X)\n",
 ...
 * Returns: 12-bit encoded immediate: bits 11:8 = rotate, bits 7:0 = imm8
 */
static u32 encode_immediate(u32 value) {
  u8 rotate = 0;
  ...
  for (i = 0; i < token_count && tokens[i].type != TOKEN_EOF; i++) {
    ...
    if (tokens[i].type == TOKEN_INSTRUCTION) {
      ...
      if (opcode == OP_MUL || opcode == OP_MLA) {
        ...
        if (i < token_count) { rd_mul = get_register(tokens[i].value); i++; }
        ...
        if (opcode == OP_MLA) {
          if (i < token_count && tokens[i].type == TOKEN_COMMA) { i++; }
          if (i < token_count) { rn_mul = get_register(tokens[i].value); i++; }
        }
        /*
         * Multiply format per REFERENCE.md:
         *   31:24 opcode | 23:20 cond | 19:16 rn | 15:12 rd | 11:8 rm | 7:4 rs | 3:0 -
         */
        instr = (opcode << 24) | (parse_condition(condition) << 20) | (rn_mul << 16) | (rd_mul << 12) | (rm << 8) | (rs << 4);
        emit_instr(prog, instr);
        continue;
      }
        ...
        * =====================================================
        */
       if (opcode == OP_LDR && i + 3 < token_count &&
         ...
         if (label_addr >= 0) {
           ...
           if (prog->literal_pool_ref_count < MAX_LITERAL_POOL_REFS) {
             prog->literal_pool_refs[prog->literal_pool_ref_count].instr_addr = prog->text_size * 4;
             prog->literal_pool_refs[prog->literal_pool_ref_count].pool_index = pool_index;
            ...
            instr = (OP_LDR << 24) | (parse_condition(condition) << 20) | (15 << 16) | (rd << 12);
            fprintf(stderr, "DEBUG: =label emit: instr=0x%08X (opcode=0x%02X, cond=0x%X, rn=0x%X, rd=0x%X)\n",
                    instr, (instr >> 24) & 0xFF, (instr >> 20) & 0xF, (instr >> 16) & 0xF, (instr >> 12) & 0xF);
            emit_instr(prog, instr);

           fprintf(stderr, "DEBUG: =label: pool_index=%u, instr_addr=%u\n",
                   pool_index, prog->text_size * 4 - 4);
         } else {
      ...
      if (opcode == OP_LDR || opcode == OP_LDRB || opcode == OP_STR || opcode == OP_STRB) {
        ...
        instr = (opcode << 24) | (parse_condition(condition) << 20) | (rn_ldr << 16) | (rt << 12) | offset_ldr;
        emit_instr(prog, instr);
        continue;
      }
