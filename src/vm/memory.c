...
u32 vm_mem_read32(vm_state_t *vm, u32 addr) {
  ...
  return (u32)vm->mem.memory[addr] |
         ((u32)vm->mem.memory[addr + 1] << 8) |
         ((u32)vm->mem.memory[addr + 2] << 16) |
         ((u32)vm->mem.memory[addr + 3] << 24);
}

uint16_t vm_mem_read16(vm_state_t *vm, u32 addr) {
  if (addr >= MEMORY_SIZE - 1) {
    return 0;
  }
  return (uint16_t)vm->mem.memory[addr] |
         ((uint16_t)vm->mem.memory[addr + 1] << 8);
}

uint8_t vm_mem_read8(vm_state_t *vm, u32 addr) {
  if (addr >= MEMORY_SIZE) {
    return 0;
  }
...
void vm_mem_write32(vm_state_t *vm, u32 addr, u32 value) {
  if (addr >= MEMORY_SIZE - 3) {
    return;
  }
...
void vm_mem_write16(vm_state_t *vm, u32 addr, uint16_t value) {
  if (addr >= MEMORY_SIZE - 1) {
    return;
  }
  vm->mem.memory[addr] = value & 0xFF;
...
void vm_mem_write8(vm_state_t *vm, u32 addr, uint8_t value) {
  if (addr >= MEMORY_SIZE) {
    return;
  }
